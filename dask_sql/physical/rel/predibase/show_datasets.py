...
class ShowDatasetsPlugin(BaseRelPlugin):
    """
    Show all tables currently defined for a given schema.
    The SQL is:

        SHOW DATASETS FROM <schema>
    ...
    class_name = "com.predibase.pql.parser.SqlShowDatasets"
    ...
    def convert(
        ...
        df = pd.DataFrame({"dataset_name": list(context.schema[schema].tables.keys())})
