...
cargo clippy --manifest-path ./dask_planner/Cargo.toml
...
cargo fmt --manifest-path ./dask_planner/Cargo.toml --all -q -- --check 2>/dev/null
if [ $? -ne 0 ]; then
    cargo fmt --manifest-path ./dask_planner/Cargo.toml --all
