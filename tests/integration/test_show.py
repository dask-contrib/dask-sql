...
class ShowTestCase(DaskTestCase):
    def test_schemas(self):
        ...
        expected_df = pd.DataFrame({"Schema": ["schema"]})

        assert_frame_equal(df, expected_df)

    def test_tables(self):
        df = self.c.sql('SHOW TABLES FROM "schema"')
        df = df.compute()
    ...
    def test_columns(self):
        df = self.c.sql('SHOW COLUMNS FROM "schema"."user_table_1"')
        df = df.compute()
    ...
    def test_wrong_input(self):
        ...
        self.assertRaises(KeyError, self.c.sql, 'SHOW COLUMNS FROM "schema"."table"')
