...
@pytest.mark.parametrize("split_every,expected_keys", [(2, 74), (3, 68), (4, 64)])
def test_groupby_split_every_cpu(c, split_every, expected_keys):
...
def test_groupby_split_every_gpu(c, split_every, expected_keys):
    # sometimes groupby will rely on dask-cudf's codepath, resulting in a different task graph
    cudf = pytest.importorskip("cudf")
    input_ddf = dd.from_pandas(
        cudf.DataFrame({"user_id": [1, 2, 3, 4] * 16, "b": [5, 6, 7, 8] * 16}),
        npartitions=16,
    )
